...
class POD():
    ...
    def init(self):
        ...
        for item in bytes:
            resp = self.handle.controlMsg(0x40, 0x67, 0, value=0x0821, index=0xF000 | item)
            print "resp (ACK)", formathex(resp)
            resp = self.handle.controlMsg(0xC0, 0x67, 1, value=0x12, index=0x00)
            print "resp (x12)", formathex(resp)
            resp = self.handle.controlMsg(0xC0, 0x67, 8, value=0x13, index=0x00)
    ...
    def get_serial_number(self):
	  ...
	  #serial number goes here
          reqType = 0xc0 #usb.TYPE_CLASS | usb.RECIP_INTERFACE | usb.ENDPOINT_OUT
          resp = self.handle.controlMsg(reqType, 0x67, 4, value=0x0013, index=0x00)
          if len(resp) > 0:
              print "serial number:", formathex(resp), " -> ",struct.unpack('I',chr(resp[0])+chr(resp[1])+chr(resp[2])+chr(resp[3]))[0]

          #todo
          reqType = 0x40 #usb.TYPE_CLASS | usb.RECIP_INTERFACE | usb.ENDPOINT_OUT
          resp = self.handle.controlMsg(reqType, 0x67, 0, value=0x11, index=0)
          if len(resp) > 0:
              print "resp1", formathex(resp)

          #todo - probably firmware version
          reqType = 0xc0 #usb.TYPE_CLASS | usb.RECIP_INTERFACE | usb.ENDPOINT_OUT
          resp = self.handle.controlMsg(reqType, 0x67, 3, value=0x11, index=0)
          if len(resp) > 0:
              print "resp2", formathex(resp)

          #todo
          reqType = 0x40 #usb.TYPE_CLASS | usb.RECIP_INTERFACE | usb.ENDPOINT_OUT
          resp = self.handle.controlMsg(reqType, 0x67, 0, value=0x0421, index=0x80d0)
          if len(resp) > 0:
              print "resp3", formathex(resp)

          #todo
          reqType = 0x40 #usb.TYPE_CLASS | usb.RECIP_INTERFACE | usb.ENDPOINT_OUT
          resp = self.handle.controlMsg(reqType, 0x67, 0, value=0x0421, index=0x80d0)
          if len(resp) > 0:
              print "resp4", formathex(resp)

          #todo
          reqType = 0xc0 #usb.TYPE_CLASS | usb.RECIP_INTERFACE | usb.ENDPOINT_OUT
          resp = self.handle.controlMsg(reqType, 0x67, 1, value=0x0012, index=0x00)
          if len(resp) > 0:
              print "resp5", formathex(resp)
